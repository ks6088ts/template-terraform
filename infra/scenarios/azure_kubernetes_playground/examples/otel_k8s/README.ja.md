...
> [!TIP]
> `kubectl run --rm -it` を使うと、コマンド終了後に Pod が自動的に削除されます。送信後に Jaeger UI や Prometheus UI で `my-test-service` や `curl-test` をサービス名として検索すると結果を確認できます。
...
1. **Namespace**: `namespace.yaml` を開き、`metadata.labels` に設定されている `app.kubernetes.io/part-of: otel-stack` が他のリソースにも共通して付与されていることを確認します。
2. **Deployment と Service の関係**: `otel-collector.yaml` を例に、Deployment の `spec.selector.matchLabels` と Service の `spec.selector` が同じラベルで紐付いていることを確認します。
3. **ConfigMap のマウント**: `otel-collector.yaml` の `volumeMounts` と `volumes` セクションを見て、ConfigMap の内容がコンテナのどのパスにマウントされるか確認します。
...
service:
  pipelines:
    traces:
      ...
      exporters: [otlp/jaeger, debug]  # ← Jaeger + 標準出力に送信
    metrics:
      receivers: [otlp]
      processors: [batch]
...
* `debug` エクスポーターにより、全テレメトリが Collector の標準出力にも出力される
...
現在のスタックではログは `debug` エクスポーター (標準出力) のみに出力されます。
...
* 現在の構成ではログバックエンドがないため `debug` exporter で標準出力に出力される
...
> [!NOTE]
> `jaegertracing/all-in-one` や `otel/opentelemetry-collector-contrib` イメージには `wget` や `curl` が含まれていない場合があるため、一時 Pod を使ってクラスタ内からエンドポイントにアクセスしています。
...
> [!NOTE]
> `kubectl edit` で直接編集する方法もありますが、マニフェストファイルを編集して `kubectl apply -f` で適用する方法を推奨します。変更がファイルとして残るため、Git での管理や変更履歴の追跡が容易になります。
...
* [OpenTelemetry Collector の構成](https://opentelemetry.io/docs/collector/configuration/)
