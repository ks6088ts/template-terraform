...
```mermaid
flowchart TB
    subgraph VNet["Spoke VNet (10.1.0.0/16)"]
        subgraph BastionSubnet["AzureBastionSubnet<br/>10.1.0.0/26"]
            Bastion["🛡️ Bastion Host"]
        end
        subgraph PaaSSubnet["PaaS Subnet<br/>10.1.1.0/24"]
            PE["🔒 Private Endpoint<br/>(Blob)"]
        end
        subgraph VMSubnet["VM Subnet<br/>10.1.2.0/24"]
            VM["🖥️ VM<br/>(Ubuntu)"]
        end
    end

    Storage[("📦 Storage Account<br/>(Private Only)")]
